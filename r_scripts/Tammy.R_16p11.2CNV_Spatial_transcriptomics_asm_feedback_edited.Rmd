...
The purpose of this code is to probe Smrithi's 16p MRI data (comparing brain size of DK parcellations in individuals with 16p11.2 duplications and deletions to healthy controls, using a spline-based model). I did a literature search and found 3 papers containing sequencing data on various 16p11.2 CNV human models (lymphoblastoid cell lines, human derived iPSC, cortical organoids). I found 3 articles:

-2014 Talkowski et al. | Transcriptional Consequences of 16p11.2 Deletion and Duplication in Mouse Cortex and Multiplex Autism Families
-2020 Roth et al. | 16p11.2 microdeletion imparts transcriptional alterations in human iPSC-derived models of early neural development
-2021 Urresti et al. | Cortical organoids model early brain development disrupted by 16p11.2 copy number variants in autism

I downloaded available differentially expressed genes data for the 16p deletion and duplication (if available).

In the following code, I analyzed the deletion data. I first organized the data from these 3 sources, then combined them into one dataframe (log-fold-change was weighted by p-values, and p-values were combined with fishers method).

I then filtered the data by significance of the combined p-values, only keeping genes with a combined p-value of p<0.001, and moved forward with analysis for these genes.
Of note, 21 out of the 27 genes in the 16p11.2 locus were in this gene list. I did a quick search and found that the following genes were not included in the p<0.001 list, and here is why:

C16orf54: not significant at all (p=0.74)
C16orf92: not significant at all (p=0.40)
SPN: significant at p<0.01 level
TBX6: significant at p<0.01 level
ZG16: NOT IN DEG LIST

I then did gene ontology analysis on all p<0.001 significant genes. Some genes were not included in the analysis, because the geneID was ambiguous (i.e. multiple entries for one gene name) or not found in their database. I tried to troubleshoot by identifying the ensembl ID for genes from the Talkowski 2014 paper, using the chromosome number, start, and stop positions given in the paper, but this did not work, likely because a different reference genome was used. A future QC step is to actually get the ensembl ID for these genes, so that all significant genes can be put through the GO analysis.

Next, I mapped the p<0.001 significant genes to the Allen Human Brain Atlas spatial map. I first used the raw expression values, and then used the normed AHBA expression values. The normed values were used in all further analysis steps

I then mapped the normalized AHBA spatial expression data to Smrithi's MRI data. For each gene, I did a basic correlation (pearson, and also spearman) of the gene expression using the t-value and the correlation coefficient (estimate). I need to ask Smrithi what value is better to use.


10/22 NEXT STEPS/TO DOS:
- finish qcing this document
- plot GO analysis, like Jakob wanted it to look
- gene name/ensembl ID link up for GO analysis
  - first make sure GOST function can take ensembl IDs
- ask Smrithi which value I should use for correlation: estimate or t-value
- next steps: i could also do the duplication data!
...
chromosomal start and stop positions and chromosome number, I was unable to find the ensembl ID for the genes in that dataset. I want to troubleshoot this further so that all genes can be put through the GO analysis.




Now, I want to fill in the "Present.in.the.16p11.2.Region" column for all data frames, to keep track of which genes are in the 16p11.2 locus
```{r}
...
Now I will do a meta-analysis to combine every instance of a gene into one observation
...
Gene ontology analysis, using gprofiler2 to do my GO analysis
...
I want to graph the GO analysis results!!!!!!!!!!!!!!!!
...
Now, I will generate a map of our significant genes using AHBA spatial transcriptomic data
...
```
NOTE: Only 87 of the 132 genes mapped to the AHBA dataset: TO TROUBLESHOOT
...
Pearson/Spearman correlation analysis between the AHBA expression and the MRI data

Making functions to compute correlation and p-value, created by ChatGPT
```{r}
...
~/Desktop/aaron_lab_2024/r_scripts/Tammy.R_16p11.2CNV_Spatial_transcriptomics_asm_feedback.Rmd

Doing the above plotting for the NON-16p genes only
```{r}
